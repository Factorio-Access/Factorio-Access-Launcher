...
class MapSettings(object):
    ...
    def reinitialize(self):
        ...
        self.map_settings()
        self.json_files = {"basic_settings": self.mgsj, "advanced_settings": self.msj}
    ...
    def map_types(self):
        self.map_types = {}
        for name, exp in self.data["noise-expression"].items():
            if "intended_property" in exp and exp["intended_property"] == "elevation":
                self.map_types[("noise-expression." + name,)] = name

    def map_gen_settings(self):
        self.mgsj = {  # Map Gen Settings Json file
            ...
            "property_expression_names": {
                "elevation": fa_menu.setting_menu_options(
                    ("gui-map-generator.map-type",), self.map_types
                ),
    ...
    def map_settings(self):
        self.msj = {  # Map Settings Json
            ...
            "pollution": {
                ...
                "min_to_diffuse": fa_menu.setting_menu_int(
                    "min_to_diffuse", None, 15, 15
                ),
    ...
    def get_presets(self, *args):
        presets = []
        for preset_group in self.data["map-gen-presets"].values():
            for name, preset in preset_group.items():
                ...
                presets.append((order, t_name, preset))
        presets.sort()
        if self.selected_preset is None:
            self.select_preset(presets[0][2])
        for i, p in enumerate(presets):
            if p[2] == self.selected_preset:
                t_name = p[1]
                if self.check_vals(p[2], self.json_files) > 0:
    ...
    def select_preset(self, preset):
        ...
        self.set_vals(preset, self.json_files)
        return 0

    def select_preset_name(self, preset):
        diffs = self.check_vals(preset, self.json_files)
        if diffs > 0:
            return ("gui-map-generator.reset-to-preset", diffs)
        return ("gui-map-generator.reset-to-preset-disabled",)

    def get_preset_desc(self, *args):
        preset = args[-1]
        return ("map-gen-preset-description." + preset["name"],)

    def launch_new(self, *args):
...
def make_menu(self):
    self.menu = {
        ...
        ("gui-map-generator.terrain-tab-title",): {
            "map_type": self.mgsj["property_expression_names"]["elevation"],
            "Water": autoplace_enable_disable_menu(
                ("gui-map-generator.water",),
                {
                    "Scale": self.mgsj["terrain_segmentation"],
                    "Coverage": self.mgsj["water"],
                },
        ...
        ("gui-map-generator.advanced-tab-title",): {
            ...
            ("gui-map-generator.map-size-group-tile",): {
                "Width": self.mgsj["width"],
                "Height": self.mgsj["height"],
            },
            ("gui-map-generator.recipes-difficulty-group-tile",): {
                "Difficulty": self.msj["difficulty_settings"]["recipe_difficulty"],
            },
            ("gui-map-generator.technology-difficulty-group-tile",): {
                "Difficulty": self.msj["difficulty_settings"]["technology_difficulty"],
                "Price multiplier": self.msj["difficulty_settings"][
            ...
            ("gui-map-generator.pollution",): enable_disable_menu(
                ...
                {
                    "Absorption modifier": self.msj["pollution"]["ageing"],
                    "Attack cost modifier": self.msj["pollution"][
                        "enemy_attack_pollution_consumption_modifier"
                    ],
                    "Minimum to damage trees": self.msj["pollution"][
                        "min_pollution_to_damage_trees"
                    ],
                    "Absorbed per damaged tree": self.msj["pollution"][
                        "pollution_restored_per_tree_damage"
                    ],
                    "Diffusion ratio": self.msj["pollution"]["diffusion_ratio"],
                },
            ),
        },
        ("gui-map-generator.play",): self.launch_new,
    }

    self.msj["pollution"]["enabled"] = self.menu[
        ("gui-map-generator.advanced-tab-title",)
    ][("gui-map-generator.pollution",)].submenu[1]

    for name, control in self.data["autoplace-control"].items():
        submenu = {}
        name = (
        ...
        if control["category"] == "resource":
            parent = self.menu[("gui-map-generator.resources-tab-title",)]
            ...
            parent = self.menu[("gui-map-generator.terrain-tab-title",)]
            ...
            parent = self.menu[("gui-map-generator.enemy-tab-title",)]
            submenu["frequency"] = fa_menu.setting_menu_float(
                ("gui-map-generator.frequency",),
        ...
        self.mgsj["autoplace_controls"][control["name"]] = submenu

    self.menu[("gui-map-generator.terrain-tab-title",)].update(
        {
            "Cliffs": autoplace_enable_disable_menu(
                ("gui-map-generator.cliffs",),
                {
                    "Frequency": self.mgsj["cliff_settings"][
                        "cliff_elevation_interval"
                    ],
                    "Continuity": self.mgsj["cliff_settings"]["richness"],
                },
            ),
            "Moisture": fa_menu.menu_item(
                ("gui-map-generator.moisture",),
                {
                    "Scale": self.mgsj["property_expression_names"][
                        "control-setting:moisture:frequency:multiplier"
                    ],
                    "Bias": self.mgsj["property_expression_names"][
    ...
    self.menu[("gui-map-generator.enemy-tab-title",)].update(
        {
            "Peaceful mode": self.mgsj["peaceful_mode"],
            "Starting area size": self.mgsj["starting_area"],
            "Enemy Expansion": enable_disable_menu(
                ("gui-map-generator.enemy-expansion-group-tile",),
                {
                    "Maximum expansion distance": self.msj["enemy_expansion"][
                        "max_expansion_distance"
                    ],
                    "Minimum group size": self.msj["enemy_expansion"][
                        "settler_group_min_size"
                    ],
                    "Maximum group size": self.msj["enemy_expansion"][
                        "settler_group_max_size"
                    ],
                    "Minimum cooldown": self.msj["enemy_expansion"][
                        "min_expansion_cooldown"
                    ],
                    "Maximum cooldown": self.msj["enemy_expansion"][
            ...
            "Evolution": enable_disable_menu(
                ...
                {
                    "Time factor": self.msj["enemy_evolution"]["time_factor"],
                    "Destroy factor": self.msj["enemy_evolution"]["destroy_factor"],
                    "Pollution factor": self.msj["enemy_evolution"]["pollution_factor"],
                },
            ),
        }
    )

    self.mgsj["cliff_settings"]["enabled"] = self.menu[
        ("gui-map-generator.terrain-tab-title",)
    ]["Cliffs"].submenu[1]
    self.msj["enemy_expansion"]["enabled"] = self.menu[
        ("gui-map-generator.enemy-tab-title",)
    ]["Enemy Expansion"].submenu[1]
    self.msj["enemy_evolution"]["enabled"] = self.menu[
        ("gui-map-generator.enemy-tab-title",)
    ]["Evolution"].submenu[1]

    sub_preset = {
        "_desc_for_presets": self.get_preset_desc,
        self.get_presets: {
            self.select_preset_name: self.select_preset,
            ("gui-map-generator.next",): self.menu,
        },
    }

    def check_vals(self, preset, obj):
        diffs = 0
    ...
    def set_defaults(self, defs, obj):
        for name, subobj in obj.items():
            if name not in defs:
                continue
            if isinstance(subobj, dict):
