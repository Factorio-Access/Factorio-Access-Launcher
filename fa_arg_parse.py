...
used = parser.add_argument_group(
    title="Dual Use",
    description="Arguments that are passed to factorio and also modify the launcher behavior",
)
...
launch_reqs.add_argument(
    ...
    dest="launch",
    metavar="ID",
    help="join multiplayer game through steam network",
)
...
extra_args_group.add_argument(
    "--bind",
    metavar="ADDRESS[:PORT]",
    help="IP address (and optionally port) to bind to",
)
extra_args_group.add_argument("--rcon-port", metavar="N", help="Port to use for RCON")
extra_args_group.add_argument(
    "--rcon-bind", metavar="ADDRESS:PORT", help="IP address and port to use for RCON"
)
extra_args_group.add_argument(
    "--rcon-password", metavar="PASSWORD", help="Password for RCON"
)
extra_args_group.add_argument(
    "--server-settings",
    metavar="FILE",
    help="Path to file with server settings. See data/server-settings.example.json",
)
extra_args_group.add_argument(
    "--use-authserver-bans",
    metavar="BOOL",
    help="Verify that connecting players are not banned from multiplayer and inform Factorio.com about ban/unban commands.",
)
extra_args_group.add_argument(
    "--use-server-whitelist", metavar="BOOL", help="If the whitelist should be used."
)
extra_args_group.add_argument(
    "--server-whitelist", metavar="FILE", help="Path to file with server whitelist."
)
extra_args_group.add_argument(
    "--server-banlist", metavar="FILE", help="Path to file with server banlist."
)
extra_args_group.add_argument(
    "--server-adminlist", metavar="FILE", help="Path to file with server adminlist."
)
extra_args_group.add_argument(
    "--console-log",
    metavar="FILE",
    help="Path to file where a copy of the server's log will be stored",
)
extra_args_group.add_argument(
    "--server-id",
    metavar="FILE",
    help="Path where server ID will be stored or read from",
)
